...
export default {
  ...
  data: () => {
    return {
      ...
      headers: [
        ...
        { text: "Cargo", align: "left", value: "cargo.nome" },
        { text: "Criado Em", align: "left", value: "criadoEm" },
  ...
  methods: {
    ...
    editar(item) {
      this.$router.push({ name: "usuario-editar", params: { id: item.id } });
    },
    ...
    async excluir() {
      try {
        ...
        const response = await axios.delete(
          `${api}/usuarios/${this.itemSelecionado.id}`
        );
